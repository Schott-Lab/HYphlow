...
def extract_valid_tokens(text: str) -> list[str]:
    return [p for p in text.split("_") if not is_accession_junk(p)]


def clean_header_text(header: str, gene_name: str) -> str:
...
def process_fasta_worker(args):
    ...
    new_fas_name, _ = common_utils.generate_smart_filename(
        user_gene_name, "FASTA", out_dir, ".fasta", is_report=False
    )
    new_rep_name, _ = common_utils.generate_smart_filename(
        user_gene_name, "FASTA", rep_dir, ".xlsx", is_report=True
    )

    out_path = Path(new_fas_name)
    rep_path = Path(new_rep_name)
        ...
        with open(path, "r", encoding="utf-8") as f:
            for line in f:
                if line.startswith(">"):
                    ...
                    new_line = clean_header_text(line, user_gene_name)
...
def run_fasta_pipeline(fasta_files, gene_dict):
    ...
    tasks = [(fp, out_dir, rep_dir, gene_dict.get(fp, "UNKNOWN")) for fp in fasta_files]

    with concurrent.futures.ThreadPoolExecutor() as executor:
        for result in executor.map(process_fasta_worker, tasks):
            if result.get("success") and result.get("rep_path"):
                rep_path = result.pop("rep_path")
                generate_fasta_format_report(result, rep_path)
